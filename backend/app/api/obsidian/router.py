...
def create_note(
    payload: CreateNoteRequest, user: User = Depends(get_current_user), _csrf: None = Depends(require_csrf)
):
...
def delete_note(
    note_path: str, user: User = Depends(get_current_user), _csrf: None = Depends(require_csrf)
):
